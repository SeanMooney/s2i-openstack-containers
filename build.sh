...
normalize_generated_lock() {
  ...
  sed "/# This file is autogenerated/{N;d;}" "${lockfile}" | awk '
  /^--(index-url|extra-index-url|trusted-host)[[:space:]]/ { next }
  !seen_package && /^$/ { next }
  !/^#/ { seen_package = 1 }
  { print }
  ' > "${tmp}"
...
case "${ACTION}" in
  ...
  list)
    ...
    ;;
  *)
    echo "Usage: STREAM=<name> $0 {build|build-parallel|push|refs|resolve|update-sources|install-deps|list} [target ...]"
    echo ""
