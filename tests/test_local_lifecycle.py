...
class LocalLifecycleTest(unittest.TestCase):
    ...
    def test_explicit_target_resolution_does_not_infer_dependencies(self):
        ...
        self.assertIn(
            "glance/glance-api",
            lifecycle.selected_images(repo_root, "all"),
        )

    def test_all_sources_use_first_context_pin_for_shared_project(self):
        repo_root = pathlib.Path(__file__).resolve().parents[1]
        requested = lifecycle.selected_images(repo_root, "all")

        specs = lifecycle.source_specs(repo_root, requested, "master")

        requirements = next(
            spec
            for spec in specs
            if spec.canonical_name == "opendev.org/openstack/requirements"
        )
        base_pin = (
            (repo_root / "containers/base/sources.txt")
            .read_text(encoding="utf-8")
            .split()[-1]
        )
        self.assertEqual(base_pin, requirements.commit)
