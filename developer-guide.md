...
The last workflow logs in to Quay, but its push step is commented out. It does
not currently publish images. Do not describe successful GitHub login or image
builds as a production publication path.
...
silently rewritten, and source URLs containing credentials are rejected.
...
base when no external project changed. A direct Watcher change selects that
same image. A direct Cyborg change selects both Cyborg images because their
shared project manifest supplies source to both contexts. Explicit Watcher plus
a changed Cyborg project produces Watcher followed by the two inferred Cyborg
...
To restrict work to a project:

```console
STREAM=master TARGET=watcher tox -e update-sources
```
